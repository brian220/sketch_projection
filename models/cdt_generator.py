...
class TreeGCN(nn.Module):
    ...
    def forward(self, tree, w):
        ...
        if self.activation:
            ...
            # adaIN
            branch *= w[self.depth][:, :branch.size(1)].unsqueeze(2)
            branch += w[self.depth][:, branch.size(1):].unsqueeze(2)
            branch = self.leaky_relu(branch)
        tree.append(branch)
        return tree, w
...
class Generator(nn.Module):
    ...
    def forward(self, z):
        tree = [z.unsqueeze(1)]
        w = []
        for adaptive_module in self.adaptive:
            w.append(adaptive_module(z))
        feat = self.gcn(tree, w)
        self.pointcloud = feat[0][-1]
