...
def test_opt_net(cfg):
    ...
    print('[INFO] %s Loading reconstruction weights from %s ...' % (dt.now(), cfg.TEST_OPT_NET.RECONSTRUCTION_WEIGHTS))
    rec_checkpoint = torch.load(cfg.TEST_OPT_NET.RECONSTRUCTION_WEIGHTS)
    ...
    output_dir = os.path.join(cfg.TEST_OPT_NET.OUT_PATH, '%s')
    ...
    TEST_OPT_NET_model_path = output_dir % 'rec_model.npy'
    rec_pc = net(rendering_images, init_point_clouds)
    np_rec_pc = rec_pc[0].detach().cpu().numpy()
    print(np_rec_pc.shape)
    np.save(TEST_OPT_NET_model_path, np_rec_pc)
    
    batch_pc = []
    pc = np.load(TEST_OPT_NET_model_path)
    batch_pc.append(pc)
    ...
    for opt_idx in range(100):
        ...
        total_loss, loss_2d, loss_3d = net.module.learn(rendering_images, init_point_clouds, batch_pc, batch_model_x, batch_model_y, batch_model_gt)
        ...
        rendering_views = utils.point_cloud_visualization.get_point_cloud_image(g_pc, os.path.join(img_dir, 'test opt net'),
                                                                                        opt_idx, epoch_idx, "reconstruction")
        opt_writer.add_image('Test Opt Net Sample/Point Cloud Reconstructed', rendering_views, opt_idx)
