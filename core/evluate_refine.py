...
def evaluate_net(cfg):
    ...
    # Load weight
    # Load weight for encoder, decoder
    print('[INFO] %s Loading refine weights from %s ...' % (dt.now(), cfg.EVALUATE.REFINE_WEIGHTS))
    ref_checkpoint = torch.load(cfg.EVALUATE.REFINE_WEIGHTS)
    ref_net.load_state_dict(ref_checkpoint['net'])
    print('[INFO] Best refine result at epoch %d ...' % ref_checkpoint['epoch_idx'])
    epoch_id = int(ref_checkpoint['epoch_idx'])
...
def evaluate_on_img(cfg, net, input_img_path, eval_transforms, eval_id, view_id, epoch_id, gt_point_cloud):
    ...
    with torch.no_grad():
        ...
        # Predict Pointcloud
        g_pc = refine_pc[0].detach().cpu().numpy()
        rendering_views = utils.point_cloud_visualization.get_point_cloud_image(g_pc,
                                                                                os.path.join(cfg.EVALUATE.OUTPUT_FOLDER, 'refine'),
                                                                                view_id, epoch_id, "refine")
